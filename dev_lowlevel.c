...
static uint8_t videoPattern[UsbFullSpeedBufferSize];
...
void usb_set_device_configuration(__unused volatile struct usb_setup_packet *pkt)
{
    ...
    uint32_t sendDataSize = fillVideoBufferWithColorRgb24(videoPattern, 0xFF, 0x5F, 0x1F);
    usb_start_transfer(ep, videoPattern, sendDataSize);

    // Set LED to indicate USER first packet ready
...
// This callback occurs after each USB transmission
...
void ep1_in_handler(uint8_t *buf, uint16_t len)
{
    ...
    if (VideoCounter > 97280 && isBufferProcessed() && isSubBufferProcessed())
    {
        switch (ColorCounter)
        {
        case 0:
    ...
    uint32_t sendDataSize = fillVideoBufferWithColorRgb24(videoPattern, Red, Green, Blue);
    struct usb_endpoint_configuration *ep = usb_get_endpoint_configuration(EP1_IN_ADDR);
    usb_start_transfer(ep, videoPattern, sendDataSize);
